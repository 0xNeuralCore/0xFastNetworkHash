...
/*
 * MemPool_alloc
 * Aligne bump_index avant chaque allocation.
 * item_size est déjà aligné en entrée (fait dans F_api_init_data).
 */
void *MemPool_alloc(MemPool *mp, size_t item_size, int history_count)
{
    size_t aligned_start = ALIGN_UP(mp->bump_index, API_ALIGN);
    size_t block_size    = item_size * (size_t)history_count;

    if (aligned_start + block_size > API_POOL_SIZE)
        return NULL;

    void *ptr      = &mp->buffer[aligned_start];
    mp->bump_index = aligned_start + block_size;
    return ptr;
}
                         ...
                         void *block, size_t item_size, int history_count)
{
    ...
    e->id            = id;
    e->block         = block;
    e->item_size     = item_size;   /* déjà aligné                          */
...
int F_api_init(void)
{
    ...
    printf("[API] init — pool %d octets / align %d\n",
           API_POOL_SIZE, API_ALIGN);
    return API_OK;
}
 ...
 * F_api_init_data
 *
 * item_size est aligné ici, une seule fois.
 * Tout le reste du code utilise cette valeur alignée
 * sans avoir besoin d'un second champ.
 */
                    ...
                    size_t item_size, int history_count)
{
    ...
    /* Alignement de item_size — seul endroit où ALIGN_UP est appelé */
    size_t item_aligned = ALIGN_UP(item_size, API_ALIGN);

    /* 1. MemPool : réserve le bloc */
    void *block = MemPool_alloc(&_mempool, item_aligned, history_count);
    if (!block) return API_ERR_FULL;

    /* 2. DataHandler : enregistre avec item_size aligné */
    int rc = DataHandler_register(&_handler, id, name,
                                  block, item_aligned, history_count);
    if (rc != API_OK) return rc;

    printf("[API] déclaré  id=%-2d  name=%-20s  "
           "size=%zu→%zu  hist=%d  pool=%zu/%d\n",
           id, name, item_size, item_aligned,
           history_count, _mempool.bump_index, API_POOL_SIZE);
